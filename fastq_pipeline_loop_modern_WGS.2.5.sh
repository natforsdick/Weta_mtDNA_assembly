# Adapted 20/8/19 by Natalie Forsdick for WGS data to run on NeSI
...
#variables
# reference file
reffile=2021-01-22_Circularized_assembly_1_Dheteracantha_Dfallai_ref_corrected.fasta
# path to reference file
refdir=/nesi/project/ga03048/weta/mtdna/
ref=$refdir$reffile

# path to raw fastq data
datadir=/nesi/nobackup/ga03048/data/illumina/subsamp/
home=/nesi/nobackup/ga03048/Weta_NOVO/mtdna_mapping/
# suffix pattern of the first fastq file
fq1=_R1_001.fastq
# suffix pattern of the first fastq file
fq2=_R2_001.fastq
# list of samples (separated by spaces, with one pair of quotes around all)
samplist="subsamp_H07456-L1_S1_L004"
...
#mkdir for sample
#mkdir ${home}${samp}_process
#cd ${home}${samp}_process
...
bwa mem -M -R @RG'\t'$rgid'\t'$rgpl'\t'$rgpu'\t'$rgsm $ref $alignfastq1 $alignfastq2 | java -jar $picard SortSam INPUT=/dev/stdin OUTPUT=${home}${samp}_sort.bam SORT_ORDER=coordinate TMP_DIR=./tmp
...
java -Xmx4g -jar $picard CleanSam I=$alignedbam O=${home}${samp}_allsortclean.bam CREATE_INDEX=true
